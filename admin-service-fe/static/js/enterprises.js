...
(() => {
  const { $, toast, apiFetch, badgeStatus, initPage, filterList, fmt } = window.LabOdcAdmin;

  initPage({
    activeNav: "enterprises",
    title: "Enterprises",
    chips: ["GET /admin/enterprise", "GET /admin/enterprise/{id}", "POST /{id}/approve", "POST /{id}/suspend"]
  });
  ...
  view.innerHTML = `
    <div class="grid">
      <div class="card">
        <div class="hd"><div class="title">Danh sách doanh nghiệp</div><div class="muted">Quản lý & duyệt</div></div>
        <div class="bd">
          <div class="toolbar">
            <div class="left">
  ...
  function render(list){
    const rows = (list||[]).map(x => `
      <tr>
        ...
        <td class="muted">${fmt(x.createdAt)}</td>
        <td>
          <div class="actions">
  ...
  async function load(force=false){
    try{
      const res = await apiFetch("/admin/enterprise");
      items = res.data || [];
      render(filterList(items, q.value));
      if(force) toast("Đã refresh", `Enterprises: ${items.length}`, "ok");
    }catch(e){
      toast("Lỗi tải enterprises", e.message, "bad");
    }
  }
  ...
  tbl.addEventListener("click", async (ev) => {
    ...
    try{
      if(act === "approve"){
        await apiFetch(`/admin/enterprise/${id}/approve`, {method:"POST"});
        toast("Approve thành công", id, "ok");
        await load();
      } else if(act === "suspend"){
        await apiFetch(`/admin/enterprise/${id}/suspend`, {method:"POST"});
        toast("Suspend thành công", id, "warn");
        await load();
      } else if(act === "detail"){
        const res = await apiFetch(`/admin/enterprise/${id}`);
        const d = res.data || {};
        toast("Enterprise detail", `Name: ${d.name || ""}\nEmail: ${d.email || ""}\nStatus: ${d.status || ""}`, "ok");
      }
    }catch(e){
