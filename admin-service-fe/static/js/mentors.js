...
(() => {
  ...
  initPage({
    activeNav: "mentors",
    title: "Mentors",
    chips: ["GET /admin/mentor", "GET /admin/mentor/{id}", "POST /{id}/activate", "POST /{id}/suspend"]
  });
  ...
  view.innerHTML = `
    <div class="grid">
      <div class="card">
        <div class="hd"><div class="title">Danh sách mentor</div><div class="muted">Activate / Suspend</div></div>
        <div class="bd">
          <div class="toolbar">
            <div class="left">
              <input class="search" id="q" placeholder="Search name/status..." />
  ...
  async function load(force=false){
    try{
      const res = await apiFetch("/admin/mentor");
      items = res.data || [];
      render(filterList(items, q.value));
      if(force) toast("Đã refresh", `Mentors: ${items.length}`, "ok");
    }catch(e){
      toast("Lỗi tải mentors", e.message, "bad");
    }
  }
  ...
  tbl.addEventListener("click", async (ev) => {
    ...
    try{
      if(act === "activate"){
        await apiFetch(`/admin/mentor/${id}/activate`, {method:"POST"});
        toast("Activate thành công", id, "ok");
        await load();
      } else if(act === "suspend"){
        await apiFetch(`/admin/mentor/${id}/suspend`, {method:"POST"});
        toast("Suspend thành công", id, "warn");
        await load();
      } else if(act === "detail"){
        const res = await apiFetch(`/admin/mentor/${id}`);
        const d = res.data || {};
        toast("Mentor detail", `Full name: ${d.fullName || ""}\nStatus: ${d.status || ""}`, "ok");
      }
    }catch(e){
